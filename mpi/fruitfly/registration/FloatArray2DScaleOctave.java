...
//	}
	...
	public FloatArray2D[] getL1( int i )
	{
		if ( l1[ i ] == null )
		{
			l1[ i ] = ImageFilter.createGradients( l[ i ] );
		}
		return l1[ i ];
	}
...
* @param initial_sigma inital gaussian sigma
			...
			float initial_sigma )
	{
		...
		for ( int i = 1; i < STEPS + 3; ++i )
		{
			...
			KERNEL_DIFF[ i ] = ImageFilter.createGaussianKernel1D(
					SIGMA_DIFF[ i ],
					true );
		}
	...
	public void buildStub()
	{
		...
		l[ 0 ] = img;
		l[ 1 ] = ImageFilter.convolveSeparable( l[ 0 ], KERNEL_DIFF[ STEPS ], KERNEL_DIFF[ STEPS ] );
