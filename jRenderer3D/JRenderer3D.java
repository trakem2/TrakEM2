...
//						v_ = 0xFF000000 |(v_ << 8);
	...
	private void finishAndDrawText() {
			...
			if(text3D != null){
				...
				for (int i = 0; i < text3D.size(); i++) {
					...
					if (text3D.get(i) != null && text3D.get(i) instanceof Text3D) {
						...
						font = new Font("Sans", Font.PLAIN, (int)(scale*ti.size));
						g2D.setFont(font);

						g2D.drawString(ti.text, x, y); 
					}
				}
			}
		}
		...
		g2D.setFont(font);
		g2D.drawString("ImageJ3D", bufferWidth - 65, bufferHeight - 5); 
...
//		this.lines3D.addAll(Arrays.asList(lines3D));
	...
	*************************************************************/
	...
	public void setTransformMaxDistance(int maxDistance) {
