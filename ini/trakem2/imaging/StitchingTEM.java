...
public class StitchingTEM {
	...
	static public ImageProcessor makeStripe(final Patch p, final Roi roi, final float scale, boolean quality, boolean ignore_patch_transform) {
		...
		if (scale < 1) {
			...
			ip.setPixels(
					ImageFilter.computeGaussianFastMirror(
							new FloatArray2D(
									( float[] )ip.getPixels(),
									ip.getWidth(),
									ip.getHeight()),
									( float )Math.sqrt( 0.25 / scale / scale - 0.25 ) ).data );
//									0.25f/scale).data); // scaling with area averaging is the same as a gaussian of sigma 0.5/scale and then resize with nearest neightbor
