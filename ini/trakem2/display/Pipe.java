...
public class Pipe extends ZDisplayable {
	...
	public Pipe(Project project, long id, String title, double x, double y, double width, double height, float alpha, boolean visible, Color color, boolean locked) {
	...
	public void setTransform(Transform t) {
		...
		if (t.width != this.width || t.height != this.height) {
			...
			// scale center for other transformations (rotate)
		}
		...
		if (0 != t.rot || t.width != this.width || t.height != this.height) {
			...
			t.y = this.y + (t.height - t.height/sy)/2 + dy;
