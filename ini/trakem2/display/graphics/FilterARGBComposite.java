 ...
 * @version 0.1b
 */
public class FilterARGBComposite implements Composite
{
	...
	final static private class ARGB2ARGB implements Composer
	{
		final public void compose( final int[] src, final int[] dst, final float alpha )
		{
			...
			dst[ 0 ] = Util.round( dst[ 0 ] * ( 255.0f - ( 255.0f - src[ 0 ] ) * srcAlpha ) / 255.0f );
			dst[ 1 ] = Util.round( dst[ 1 ] * ( 255.0f - ( 255.0f - src[ 1 ] ) * srcAlpha ) / 255.0f );
			dst[ 2 ] = Util.round( dst[ 2 ] * ( 255.0f - ( 255.0f - src[ 2 ] ) * srcAlpha ) / 255.0f );
			dst[ 3 ] = 255;
		}
	}
	final static private class RGB2ARGB implements Composer
	{
		final public void compose( final int[] src, final int[] dst, final float alpha )
		{
			dst[ 0 ] = Util.round( dst[ 0 ] * ( 255.0f - ( 255.0f - src[ 0 ] ) * alpha ) / 255.0f );
			dst[ 1 ] = Util.round( dst[ 1 ] * ( 255.0f - ( 255.0f - src[ 1 ] ) * alpha ) / 255.0f );
			dst[ 2 ] = Util.round( dst[ 2 ] * ( 255.0f - ( 255.0f - src[ 2 ] ) * alpha ) / 255.0f );
			dst[ 3 ] = 255;
		}
	}
	final static private class Gray2ARGB implements Composer
	{
		...
		{
			dst[ 0 ] = Util.round( dst[ 0 ] * ( 255.0f - ( 255.0f - src[ 0 ] ) * alpha ) / 255.0f );
			dst[ 1 ] = Util.round( dst[ 1 ] * ( 255.0f - ( 255.0f - src[ 0 ] ) * alpha ) / 255.0f );
			dst[ 2 ] = Util.round( dst[ 2 ] * ( 255.0f - ( 255.0f - src[ 0 ] ) * alpha ) / 255.0f );
			dst[ 3 ] = 255;
		}
	}
	
	static private FilterARGBComposite instance = new FilterARGBComposite();

	final private float alpha;

	public static FilterARGBComposite getInstance( final float alpha )
	{
		if ( alpha == 1.0f ) { return instance; }
		return new FilterARGBComposite( alpha );
	}

	private FilterARGBComposite()
	{
		this.alpha = 1.0f;
	}

	private FilterARGBComposite( final float alpha )
	{
		this.alpha = alpha;
	}
