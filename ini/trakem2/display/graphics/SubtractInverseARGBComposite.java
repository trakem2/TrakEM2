 ...
 * @version 0.1b
 */
public class SubtractInverseARGBComposite implements Composite
{
	...
	final static private class ARGB2ARGB implements Composer
	{
		final public void compose( final int[] src, final int[] dst, final float alpha )
		{
			...
			dst[ 0 ] = Math.max( 0, Util.round( dst[ 0 ] - ( 255 - src[ 0 ] ) * srcAlpha ) );
			dst[ 1 ] = Math.max( 0, Util.round( dst[ 1 ] - ( 255 - src[ 1 ] ) * srcAlpha ) );
			dst[ 2 ] = Math.max( 0, Util.round( dst[ 2 ] - ( 255 - src[ 2 ] ) * srcAlpha ) );
			dst[ 3 ] = 255;
		}
	}
	final static private class RGB2ARGB implements Composer
	{
		final public void compose( final int[] src, final int[] dst, final float alpha )
		{
			dst[ 0 ] = Math.max( 0, Util.round( dst[ 0 ] - ( 255 - src[ 0 ] ) * alpha ) );
			dst[ 1 ] = Math.max( 0, Util.round( dst[ 1 ] - ( 255 - src[ 1 ] ) * alpha ) );
			dst[ 2 ] = Math.max( 0, Util.round( dst[ 2 ] - ( 255 - src[ 2 ] ) * alpha ) );
			dst[ 3 ] = 255;
		}
	}
	final static private class Gray2ARGB implements Composer
	{
		...
		{
			dst[ 0 ] = Math.max( 0, Util.round( dst[ 0 ] - ( 255 - src[ 0 ] ) * alpha ) );
			dst[ 1 ] = Math.max( 0, Util.round( dst[ 1 ] - ( 255 - src[ 0 ] ) * alpha ) );
			dst[ 2 ] = Math.max( 0, Util.round( dst[ 2 ] - ( 255 - src[ 0 ] ) * alpha ) );
			dst[ 3 ] = 255;
		}
	}
	
	static private SubtractInverseARGBComposite instance = new SubtractInverseARGBComposite();

	final private float alpha;

	public static SubtractInverseARGBComposite getInstance( final float alpha )
	{
		if ( alpha == 1.0f ) { return instance; }
		return new SubtractInverseARGBComposite( alpha );
	}

	private SubtractInverseARGBComposite()
	{
		this.alpha = 1.0f;
	}

	private SubtractInverseARGBComposite( final float alpha )
	{
		this.alpha = alpha;
	}
