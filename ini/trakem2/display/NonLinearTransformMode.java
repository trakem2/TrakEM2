...
public class NonLinearTransformMode implements Mode {
	...
	private class Painter extends SimpleThread
	{
		void doit( final Rectangle r, final double m )
		{
			try
			{
				...
				toWorld.set( 1.0f / ( float )m, 0, 0, 1.0f / ( float )m, r.x - ScreenPatchRange.pad / ( float )m, r.y - ScreenPatchRange.pad / ( float )m );
			...
			if ( null == p_clicked )
			{
				...
				try
				{
					if ( points.size() > 0 )
					{
						 ...
						 * TODO bug here!!!
						 * 
						 * Fixable easily by creating a TransformMesh with world size but to the cost
						 * of very coarsely inverting point locations.  Better: make a mesh for just
						 * the screen which requires concatenation of some transforms---do that tomorrow.
						 */ 
						final MovingLeastSquaresTransform mlst = createMLST();
	...
	public boolean apply() {
		...
		/* TODO Beautiful!  I will try to understand and deflate these indentations tomorrow and also fix the actual operation if I manage to find it. */
